import pygame
import random


# Initialize Pygame
pygame.init()

# Screen settings
WIDTH = 800
HEIGHT = 600
GRID_SIZE = 40
screen = pygame.display.set_mode((WIDTH, HEIGHT))
pygame.display.set_caption("Frogger Game")
camera_offset = 0
next_lane_y = -GRID_SIZE  # Start spawning above the top of the screen

small_font = pygame.font.Font(None, 36)



spawn_y = HEIGHT - GRID_SIZE  # Starting position
score = 0



# Colors
WHITE = (255, 255, 255)
RED = (255, 0, 0)
GREEN = (0, 255, 0)
BLUE = (0,0,255)
TURQUOISE = (0,255,255)
YELLOW = (255,255,0)
ORANGE = (255,165,0)
PINK = (255,192,203)
PURPLE = (128,0,128)
BROWN = (165,42,42)
BLACK = (0,0,0)
GRAY = (128,128,128)
DARK_GRAY = (169,169,169)
LIGHT_GRAY = (211,211,211)
GOLD = (255,215,0)
SILVER = (192,192,192)
BRONZE = (205,127,50)
MAROON = (128,0,0)
NAVY = (0,0,128)
OLIVE = (128,128,0)
TEAL = (0,128,128)

# Handler for the sprite sheet images
class SpriteSheet:
    def __init__(self, filename):
        self.sprite_sheet = pygame.image.load(filename).convert_alpha()

    def get_image(self, x, y, width, height):
        image = pygame.Surface((width, height)).convert()
        image.blit(self.sprite_sheet, (0, 0), (x, y, width, height))
        return image
    
# Load sprite sheet
sprite_sheet = SpriteSheet("cars.png")

# Load the start screen image
intro_image = pygame.image.load("Intro.png")
intro_image = pygame.transform.scale(intro_image, (WIDTH, HEIGHT))

# Frog
frog_image = pygame.image.load("frog.png").convert_alpha()
frog_image = pygame.transform.scale(frog_image, (GRID_SIZE, GRID_SIZE))  # Match size


# Font for start screen text
font = pygame.font.Font(None, 74)

# Extract the sprites
SPRITE_WIDTH = 40
SPRITE_HEIGHT = 100

# Extract the car sprites from the sprite sheet
car1 = sprite_sheet.get_image(250, 20, SPRITE_WIDTH, SPRITE_HEIGHT)  # First row, 5th car

car2 = sprite_sheet.get_image(190, 16, SPRITE_WIDTH, SPRITE_HEIGHT)  # First row, 4th car

car3 = sprite_sheet.get_image(130, 16, SPRITE_WIDTH, SPRITE_HEIGHT)  # First row, 3rd car

car4 = sprite_sheet.get_image(10, 440, SPRITE_WIDTH, SPRITE_HEIGHT)  # Fifth row, 1st car
car5 = sprite_sheet.get_image(74, 440, SPRITE_WIDTH, SPRITE_HEIGHT)  # Fifth row, 2nd car
car6 = sprite_sheet.get_image(137, 440, SPRITE_WIDTH, SPRITE_HEIGHT)  # Fifth row, 3rd car
car7 = sprite_sheet.get_image(200, 440, SPRITE_WIDTH, SPRITE_HEIGHT)  # Fifth row, 4th car

# Scale the car sprites to fit the grid size
#car1 = pygame.transform.scale(car1, (GRID_SIZE, GRID_SIZE))
#car2 = pygame.transform.scale(car2, (GRID_SIZE, GRID_SIZE))
#car3 = pygame.transform.scale(car3, (GRID_SIZE, GRID_SIZE))
#car4 = pygame.transform.scale(car4, (GRID_SIZE, GRID_SIZE))
#car5 = pygame.transform.scale(car5, (GRID_SIZE, GRID_SIZE))
#car6 = pygame.transform.scale(car6, (GRID_SIZE, GRID_SIZE))
#car7 = pygame.transform.scale(car7, (GRID_SIZE, GRID_SIZE))

# Rotate all car sprites 90 degrees to the right (clockwise)
car1 = pygame.transform.rotate(car1, -90)
car2 = pygame.transform.rotate(car2, -90)
car3 = pygame.transform.rotate(car3, -90)
car4 = pygame.transform.rotate(car4, -90)
car5 = pygame.transform.rotate(car5, -90)
car6 = pygame.transform.rotate(car6, -90)
car7 = pygame.transform.rotate(car7, -90)

# Player class
class Player:
    def __init__(self, x, y):
        self.x = x
        self.y = y
        self.speed = GRID_SIZE

    def move(self, direction):
        if direction == "up":
            self.y -= self.speed
        elif direction == "down" and self.y < HEIGHT - GRID_SIZE:
            self.y += self.speed
        elif direction == "left" and self.x > 0:
            self.x -= self.speed
        elif direction == "right" and self.x < WIDTH - GRID_SIZE:
            self.x += self.speed

    def draw(self):
        screen.blit(frog_image, (self.x, self.y + camera_offset))

# Obstacle class (accepting a specific sprite)
class carObstacle:
    def __init__(self, x, y, speed, sprite):
        self.x = x
        self.y = y
        self.speed = speed
        self.image = sprite  # Use the specified sprite
        self.rect = self.image.get_rect(topleft=(x, y))

    def move(self):
        self.x += self.speed
        self.rect.x = self.x
        if self.x > WIDTH:
            self.x = -GRID_SIZE
            self.rect.x = self.x
        elif self.x < -GRID_SIZE:
            self.x = WIDTH
            self.rect.x = self.x

    def draw(self):
    # Determine lane color based on Y position (you can customize this logic)
        lane_index = self.y // GRID_SIZE
        color = DARK_GRAY if lane_index % 2 == 0 else LIGHT_GRAY

        # Draw lane background rectangle
        pygame.draw.rect(screen, color, (0, self.y + camera_offset, WIDTH, GRID_SIZE))

        # Draw the car
        screen.blit(self.image, (self.x, self.y + camera_offset))


#obastacle set up
def car_obstacle(existing_obstacles, difficulty=None):
    SAFE_LANE_Y = HEIGHT - GRID_SIZE  # Bottom lane
    lane_choices = [y for y in range(0, HEIGHT, GRID_SIZE) if y != SAFE_LANE_Y]
    lane_y = random.choice(lane_choices)
    sprite = random.choice([car1, car2, car3, car4, car5, car6, car7])
    side = random.choice(["left", "right"])
    #speed = random.choice([20, 22, 24, 26, 28, 30, 32])
    speed = random.choice([2,3,4,5,6])

    speed_value = difficulty * speed 

    # If no speed is passed, speed = 10
    if speed is None:
        speed_final = 10


    # Start from left or right
    if side == "left":
        x = -GRID_SIZE
        speed_final = speed_value  # move right (positive speed)
    else:
        x = WIDTH
        speed_final = -speed_value  # move left (negative speed)

    return carObstacle(x, lane_y, speed_final, sprite)


#Grass Lane
class GrassObstacle:
    def __init__(self, y):
        self.y = y  # Y position of the lane

    def move(self):
        pass  # No movement needed

    def draw(self):
        pygame.draw.rect(screen, GREEN, (0, self.y + camera_offset, WIDTH, GRID_SIZE))


# Game setup
difficulty = 1
def initialize_game():
    player = Player(WIDTH // 2, HEIGHT - GRID_SIZE)
    obstacles = []

    total_lanes = 15
    current_y = HEIGHT - GRID_SIZE

    num_grass_lanes = random.randint(1, 5)
    for _ in range(num_grass_lanes):
        obstacles.append(GrassObstacle(current_y))
        current_y -= GRID_SIZE

    # Step 2: Fill remaining lanes with random grass or car lanes
    for _ in range(total_lanes - num_grass_lanes):
        if random.choice([True, False]):
            # Grass lane
            obstacles.append(GrassObstacle(current_y))
        else:
            # Car lane
            car = car_obstacle([], difficulty = difficulty)
            car.y = current_y
            car.rect.y = current_y
            obstacles.append(car)
        current_y -= GRID_SIZE

    return player, obstacles


# load and play the music
pygame.mixer.music.load("Cars8bit.mp3")
pygame.mixer.music.set_volume(0.5)  # Set volume to 50%
pygame.mixer.music.play(-1)  # Loop the music indefinitely


#Game Setup 
player, obstacles = initialize_game()
clock = pygame.time.Clock()

# Game state
game_state = "start"

#game timer speed interval
start_time = pygame.time.get_ticks()
speed_increase_interval = 5000  # every 5 seconds
last_speed_increase_time = start_time

# Game loop
running = True
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False
        elif event.type == pygame.KEYDOWN:
            if game_state == "start" and event.key == pygame.K_RETURN:
                game_state = "playing"
            elif game_state == "playing":
                if event.key == pygame.K_UP:
                    player.move("up")
                elif event.key == pygame.K_DOWN:
                    player.move("down")
                elif event.key == pygame.K_LEFT:
                    player.move("left")
                elif event.key == pygame.K_RIGHT:
                    player.move("right")
            elif game_state in ("game_over", "victory") and event.key == pygame.K_RETURN:
                # Reset the game by reinitializing player and obstacles
                player, obstacles = initialize_game()
                game_state = "start"    

    # Handle game states
    if game_state == "start":
        screen.blit(intro_image, (0, 0))
        camera_offset = 0
        next_lane_y = -GRID_SIZE  # Reset this to start spawning again
        # Uncomment to add "Press Enter to Start" text if needed
        # text = font.render("Press Enter to Start", True, BLACK)
        # text_rect = text.get_rect(center=(WIDTH // 2, HEIGHT // 2))
        # screen.blit(text, text_rect)

    elif game_state == "playing":
        current_time = pygame.time.get_ticks()

        # Move all obstacles
        for obstacle in obstacles:
            obstacle.move()

        # Speed up car obstacles every 5 seconds
        if current_time - last_speed_increase_time > speed_increase_interval:
            for obstacle in obstacles:
                if isinstance(obstacle, carObstacle):
                    if obstacle.speed > 0:
                        obstacle.speed += 1
                    else:
                        obstacle.speed -= 0.5
            last_speed_increase_time = current_time

        if current_time > 5000:
            camera_offset += 0.4
        #draw obstacles after first 15
        if next_lane_y + camera_offset < HEIGHT:
            if random.choice([True, False]):
                obstacles.append(GrassObstacle(next_lane_y))
            else:
                car = car_obstacle([], difficulty)
                car.y = next_lane_y
                car.rect.y = next_lane_y
                obstacles.append(car)
            next_lane_y -= GRID_SIZE


        # Update score
        score = (spawn_y - player.y) // GRID_SIZE

        # Draw background
        screen.fill(WHITE)

        # Draw score (top-right, follows camera)
        score_text = small_font.render(f"Score: {score}", True, BLACK)

        screen.blit(score_text, (WIDTH - 150, 10 - camera_offset))

        # Collision detection
        player_rect = pygame.Rect(player.x, player.y, GRID_SIZE, GRID_SIZE)
        for obstacle in obstacles:
            if isinstance(obstacle, carObstacle):
                if player_rect.colliderect(obstacle.rect):
                    game_state = "game_over"
                    break

        # Draw everything
        for obstacle in obstacles:
            obstacle.draw()
        player.draw()

            


    elif game_state == "game_over":
        screen.fill(BLACK)  # Black background for game over screen
        text = font.render("""Retry? Press enter to try again""", True, WHITE)
        text_rect = text.get_rect(center=(WIDTH // 2, HEIGHT // 2))
        screen.blit(text, text_rect)

    # ***** Added Victory Screen ******
    elif game_state == "victory":
        screen.fill(GREEN)
        text = font.render("You Win! Press Enter", True, WHITE)
        text2 = font.render("to Play Again", True, WHITE)

        text_rect = text.get_rect(center=(WIDTH // 2, HEIGHT // 2 - 20))
        text2_rect = text2.get_rect(center=(WIDTH // 2, HEIGHT // 2 + 20))

        screen.blit(text, text_rect)
        screen.blit(text2, text2_rect)

    pygame.display.flip()
    clock.tick(60)

pygame.quit()